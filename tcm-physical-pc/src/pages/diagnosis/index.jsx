  ...
  const fetchDiagnosisList = (params = {}) => {
    ...
    setTimeout(() => {
      ...
      if (params.dateRange && params.dateRange.length) {
        ...
        filteredData = filteredData.filter(
          (item) => item.visitDate >= startDate && item.visitDate <= endDate,
        );
      }
  ...
  const handleSave = () => {
    form.validateFields().then((values) => {
      ...
      const saveData = {
        ...
        appointmentNo: values.appointmentNo,
        doctorName:
          doctorOptions.find((d) => d.id === values.doctorId)?.name || '',
        ...
        <Space size='small'>
          ...
          {record.status === 'pending' && (
            <Button
              ...
              onClick={() =>
                openModal({ ...record, status: 'processing' }, 'edit')
              }
              size='small'
              style={{ color: 'green' }}>
              开始接诊
  ...
  return (
    <div className='diagnosis-page'>
      ...
      <Modal
        ...
        width={700}
        destroyOnHidden>
        <Form form={form} layout='vertical' className='diagnosis-form'>
          <Form.Item
            name='patientName'
            label='患者姓名'
