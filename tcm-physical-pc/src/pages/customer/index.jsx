...
const CustomerPage = () => {
  ...
  const handleSave = () => {
    form.validateFields().then((values) => {
      ...
      const saveData = {
        id:
          actionType === 'add'
            ? Math.max(...customerList.map((c) => c.id), 0) + 1
            : currentRecord.id,
        name: values.name,
        ...
        lastVisitDate:
          actionType === 'add'
            ...
            : currentRecord.lastVisitDate,
        visitCount: actionType === 'add' ? 0 : currentRecord.visitCount,
        totalConsumption:
          actionType === 'add' ? 0 : currentRecord.totalConsumption,
      ...
      if (actionType === 'add') {
        const exists = customerList.some(
          (item) => item.phone === saveData.phone,
        );
        if (exists) {
        ...
        const exists = customerList.some(
          (item) => item.phone === saveData.phone && item.id !== saveData.id,
        );
        if (exists) {
          message.error('该手机号已存在');
          setLoading(false);
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 120,
      render: (type) => {
        const typeInfo = customerTypeOptions.find(
          ...
          <Space size='large'>
            ...
            <div>
              ...
              <span className='stats-value'>
                ¥{customerList.reduce((sum, c) => sum + c.totalConsumption, 0)}
              </span>
            </div>
          </Space>
        </div>
            ...
            <TabPane tab='基本信息' key='info'>
              <div className='customer-basic-info'>
                <div className='customer-avatar'>
                  ...
                  <Tag
                    color={
                      currentRecord.type === 'vip'
                        ? 'red'
                        : currentRecord.type === 'member'
                          ? 'orange'
                          : 'blue'
                    }>
                    {
                      customerTypeOptions.find(
                        (t) => t.value === currentRecord.type,
                      )?.label
                    }
                  </Tag>
                </div>
