...
const CustomerPage = () => {
  ...
  const fetchCustomerList = (params = {}) => {
    ...
    setTimeout(() => {
      const mockData = [
        {
          ...
          name: '张三',
          gender: '男',
          age: 35, // 编辑/详情页用，表格不展示
          phone: '13800138000',
          type: 'member', // 编辑/详情页用，表格不展示
      ...
      // 应用筛选条件（保留客户类型筛选，供后台过滤用）
      let filteredData = [...mockData];
      if (params.name) filteredData = filteredData.filter((item) => item.name.includes(params.name));
      if (params.phone) filteredData = filteredData.filter((item) => item.phone.includes(params.phone));
      if (params.type) filteredData = filteredData.filter((item) => item.type === params.type);
      if (params.registerDateRange && params.registerDateRange.length) {
        ...
        filteredData = filteredData.filter(item => item.registerDate >= startDate && item.registerDate <= endDate);
      }
  ...
  const handleSave = () => {
    form.validateFields().then((values) => {
      ...
      const saveData = {
        ...
        // 新增时自动填充的默认值（非必填字段）
        age: actionType === 'add' ? 0 : values.age,
        type: actionType === 'add' ? 'normal' : values.type,
        registerDate: actionType === 'add' ? today : values.registerDate.format('YYYY-MM-DD'),
        lastVisitDate: actionType === 'add' ? today : currentRecord.lastVisitDate,
        visitCount: actionType === 'add' ? 0 : currentRecord.visitCount,
        totalConsumption: actionType === 'add' ? 0 : currentRecord.totalConsumption,
        address: actionType === 'add' ? '' : values.address,
        remark: actionType === 'add' ? '' : values.remark,
      };
      ...
      // 模拟保存API请求
      setTimeout(() => {
        const newList = actionType === 'add' 
          ? [...customerList, saveData] 
          : customerList.map(item => item.id === currentRecord.id ? saveData : item);
        
        setCustomerList(newList);
        message.success(actionType === 'add' ? '客户创建成功' : '客户信息更新成功');
        setVisible(false);
        setLoading(false);
      }, 500);
    });
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Space size='small'>
          <Button type='text' icon={<EyeOutlined />} onClick={() => viewCustomerDetail(record)} size='small'>详情</Button>
          <Button type='text' icon={<EditOutlined />} onClick={() => openModal(record, 'edit')} size='small'>编辑</Button>
          <Button type='text' icon={<UserAddOutlined />} onClick={() => addAppointment(record)} size='small' style={{ color: '#8B5A2B' }}>新增预约</Button>
          <Popconfirm title='确定要删除该客户吗？' onConfirm={() => deleteCustomer(record.id)} okText='确定' cancelText='取消'>
  ...
  return (
    <div className='customer-page'>
      <div className='page-header'>
        ...
        <Button type='primary' icon={<PlusOutlined />} onClick={() => openModal(null, 'add')}>新增客户</Button>
      </div>
      ...
      {/* 客户表格（已删除“性别/年龄”和“客户类型”列） */}
      <Card className='table-card'>
        <div className='stats-bar'>
          <Space size='large'>
            <div><span className='stats-label'>客户总数：</span><Badge count={customerList.length} size='large' /></div>
            <div><span className='stats-label'>会员数量：</span><Badge count={customerList.filter(c => c.type === 'member' || c.type === 'vip').length} size='large' color='orange' /></div>
            <div><span className='stats-label'>本月新增：</span><Badge count={3} size='large' color='green' /></div>
            <div><span className='stats-label'>累计消费总额：</span><span className='stats-value'>¥{customerList.reduce((sum, c) => sum + c.totalConsumption, 0)}</span></div>
          </Space>
        </div>
        <Table
          columns={columns}
      ...
      <Modal
        ...
        width={actionType === 'add' ? 400 : 600}
        destroyOnHidden>
        <Form form={form} layout='vertical' className='customer-form'>
          {/* 新增/编辑共用必填字段：姓名、性别、电话 */}
          <Form.Item
            name='name'
            label='客户姓名'
          ...
          {actionType === 'edit' && (
            <>
              ...
              <Form.Item name='registerDate' label='注册日期'>
      ...
      <Modal
        ...
        footer={[
          ...
          <Button key='edit' type='primary' onClick={() => { closeDetailModal(); openModal(currentRecord, 'edit'); }}>编辑</Button>,
        ]}>
        {currentRecord && (
            ...
            <TabPane tab='基本信息' key='info'>
              <div className='customer-basic-info'>
                <div className='customer-avatar'>
                  ...
                  <Tag color={currentRecord.type === 'vip' ? 'red' : currentRecord.type === 'member' ? 'orange' : 'blue'}>
                ...
                <Descriptions column={2} bordered>
                  ...
                  <Descriptions.Item label='最近消费'>{currentRecord.lastVisitDate}</Descriptions.Item>
                  <Descriptions.Item label='消费次数'>{currentRecord.visitCount}次</Descriptions.Item>
                  ...
                  <Descriptions.Item label='备注信息' span={2}>{currentRecord.remark || '无'}</Descriptions.Item>
                </Descriptions>
              </div>
            </TabPane>
            <TabPane tab={<><HistoryOutlined /> 消费记录</>} key='history'>
              <List
                itemLayout='vertical'
                dataSource={customerHistory}
                loading={loading && customerHistory.length === 0}
                renderItem={(item) => (
                  <List.Item key={item.id} extra={`¥${item.amount}`} className='history-item'>
                    <List.Item.Meta
                      title={
              ...
              {!loading && customerHistory.length === 0 && (
                <div className='no-history'>暂无消费记录</div>
              )}
            </TabPane>
          </Tabs>
        )}
