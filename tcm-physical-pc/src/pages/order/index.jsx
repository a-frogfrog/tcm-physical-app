...
const OrderPage = () => {
    ...
    setTimeout(() => {
      ...
      if (params.orderType) {
        filteredData = filteredData.filter(
          (item) => item.orderType === params.orderType,
        );
      }
  ...
  const openModal = (record, type) => {
    ...
    if (type === 'edit' && record) {
      ...
      form.setFieldsValue({
        customerId: customerOptions.find((c) => c.name === record.customerName)
          ?.id,
  ...
  const handleSave = () => {
    ...
    form.validateFields().then((values) => {
      ...
      const saveData = {
        id:
          actionType === 'add'
            ? `ORD${moment().format('YYYYMMDD')}${Math.floor(
                Math.random() * 100,
              )
                .toString()
                .padStart(3, '0')}`
        ...
        status: values.status,
        paymentTime:
          values.status === 'paid' && values.paymentTime
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 100,
      render: (status) => {
        const statusInfo = orderStatusOptions.find(
      ...
      <Modal
        ...
        <Form form={form} layout='vertical' className='order-form'>
          ...
          <Space style={{ width: '100%' }}>
            ...
            <Form.Item name='doctorName' label='接诊医生' style={{ flex: 1 }}>
