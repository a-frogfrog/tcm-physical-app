...
import { 
  ...
  DatePicker, Select, message, Popconfirm, Divider, 
  Checkbox, Radio ,InputNumber  
} from 'antd';
...
const SchedulePage = () => {
  ...
  const fetchScheduleList = (params = {}) => {
    ...
    setTimeout(() => {
      // 根据当前视图模式生成不同日期范围的数据
      let mockData = [];
      const baseDate = currentDate.clone().startOf(viewMode === 'week' ? 'week' : viewMode);
      
      // 生成7天（周视图）或30天（月视图）的数据
      const daysCount = viewMode === 'week' ? 7 : viewMode === 'month' ? 30 : 1;
      
      for (let d = 0; d < daysCount; d++) {
        const scheduleDate = baseDate.clone().add(d, 'days').format('YYYY-MM-DD');
  ...
  const handleSave = () => {
    form.validateFields().then(values => {
      ...
      const saveData = {
        ...
        currentAppointments: actionType === 'add' ? 0 : currentRecord.currentAppointments,
        status: values.status,
      ...
      // 检查是否已存在相同的排班
      if (actionType === 'add') {
        const exists = scheduleList.some(item => 
  ...
  const viewAppointments = (record) => {
    navigate(`/appointment?doctorId=${record.doctorId}&date=${record.scheduleDate}&shiftId=${record.shiftId}`);
  };
    ...
    },
    {
      ...
      render: (text, record) => (
        <div>
          ...
          <div style={{ fontSize: '12px', color: '#666' }}>{record.doctorSpecialty}</div>
        </div>
      )
    },
    {
      ...
      render: (text, record) => (
        <div>
          ...
          <div className="progress-bar">
            <div 
              className="progress" 
              style={{ width: `${(record.currentAppointments / record.maxAppointments) * 100}%` }}
            ></div>
          </div>
        </div>
      )
    ...
    <div className="schedule-page">
      ...
      <Card className="calendar-card">
        <div className="calendar-controls">
          <RadioGroup value={viewMode} onChange={(e) => handleViewModeChange(e.target.value)}>
          ...
          <div className="date-nav">
            ...
            <Button 
              ...
              size="small"
            >
              前{viewMode === 'day' ? '一天' : viewMode === 'week' ? '一周' : '一月'}
            </Button>
            ...
            <Button 
              ...
              size="small"
            >
              后{viewMode === 'day' ? '一天' : viewMode === 'week' ? '一周' : '一月'}
            </Button>
          </div>
